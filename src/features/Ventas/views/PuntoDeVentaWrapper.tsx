...
import PuntoDeVenta from '@/views/puntodeventa/PuntoDeVenta';
import {PuntoDeVentaMobile} from '@/features/Ventas/views/PuntoDeVentaMobile';

interface PuntoDeVentaWrapperProps {
  forceVersion?: 'mobile' | 'desktop' | 'auto';
}

const PuntoDeVentaWrapper = ({ forceVersion = 'auto' }: PuntoDeVentaWrapperProps) => {
  const isMobile = useIsMobileForPedidos();
  const [manualVersion, ] = useState<'mobile' | 'desktop' | null>(null);
  ...
  return (
    <div className="relative">
      ...
      {showMobile ? <PuntoDeVentaMobile /> : <PuntoDeVenta />}
    </div>
  );
};

export default PuntoDeVentaWrapper; 