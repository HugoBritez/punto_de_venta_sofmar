...
import type { CrearPersonaDTO } from "@/models/dtos/CrearPersonaDTO"
import { useCrearPersona } from "@/shared/hooks/mutations/personal/crearPersonal"
import { useGetCiudades } from "@/shared/hooks/queries/useCiudades"
import { useGetDepartamento } from "@/shared/hooks/queries/useDepartamentos"
import { useMonedas } from "@/shared/hooks/queries/useMonedas"
import { useZonas } from "@/shared/hooks/queries/useZonas"
import { useEffect, useState, useRef } from "react"
import { procedencias } from "../utils/procedencias"
import { useGetTipoDocumento } from "@/shared/hooks/queries/useTipoDocumento"
import { Autocomplete } from "@/shared/ui/autocomplete/AutocompleteComponent"
import { useMediaQuery, useToast } from "@chakra-ui/react"
import Tabs from "./tabs"
import { useGetGruposDeClientes } from "@/shared/hooks/queries/useGruposDeCLientes"
...
import ModalVendedores from "@/shared/ui/vendedores/ModalVendedores"
import { UsuarioViewModel } from "@/models/viewmodels/usuarioViewModel"
import { GrupoCliente } from "@/repos/grupoClienteRepository"
import { useGetTipoPlazo } from "@/shared/hooks/queries/useTipoPlazo"
import { TipoPlazo } from "@/repos/tipoPlazoRepository"
import { UsuarioRepository } from "@/repos/usuarioRepository"
import { useGetUltimoCodigoInterno } from "@/shared/hooks/queries/usePersonas"
...
const CrearPersonaForm = ({ personaAEditar }: CrearPersonaFormProps) => {
    ...
    const handleCrearPrecio = async (precio: string) => {
        if (!precio) return
        try {
            toast({
                title: "Éxito",
            ...
            toast({
                title: "Error",
                description: "No se pudo crear la lista de precios",
                status: "error",
                duration: 3000,
                isClosable: true,
            })
    ...
    const tabs = [
        {
            ...
            content: (
                <div className={isMobile ? "p-4 flex flex-col gap-4" : "p-4 flex flex-row gap-4"}>
                        ...
                        <div>
                            <Autocomplete
                                ...
                                value={
                                    Ciudades
                                        ? Ciudades
                                            .filter(ciu => ciu.id === personaDTO.persona.ciudad)
                                            .map(ciu => ({
                                                id: ciu.id,
                                                label: ciu.descripcion,
                                                value: ciu.id
                                            }))[0] ?? null
                                        : null
                                }
                                onChange={(option) => {
                                    updatePersona("ciudad", option?.value ?? 0)
                                    updateCliente("ciudad", option?.value ?? 0)
                                }}
                                disabled={!isProveedorSelected && !isClienteSelected}
                                isLoading={isLoadingCiudades}
                                isError={isErrorCiudades}
                                errorMessage={isErrorCiudades ? "Error al cargar las ciudades" : ""}
                        ...
                        <div className="flex flex-row gap-2 w-full">
                            <Autocomplete
                                ...
                                value={
                                    Zonas
                                        ? Zonas
                                            .filter(z => z.codigo === personaDTO.persona.zona)
                                            .map(z => ({
                                                id: z.codigo,
                                                label: z.descripcion,
                                                value: z.codigo
                                ...
                                onChange={(option) => {
                                    updatePersona("zona", option?.value ?? 0)
                                    updateCliente("zona", option?.value ?? 0)
                                    updateProveedor("zonaId", option?.value ?? 0)
                                }}
                                isLoading={isLoadingZonas}
                                disabled={!isProveedorSelected && !isClienteSelected}
                                isError={isErrorZonas}
                                errorMessage={isErrorZonas ? "Error al cargar zonas" : ""}
        ...
        },
        {
            ...
            disabled: !isClienteSelected,
            content: (
                <div className={isMobile ? "p-4 flex flex-col gap-4 bg-gray-100" : "p-4 flex flex-row gap-4 bg-gray-100"}>
                    <div className="flex flex-col gap w-full">
                        ...
                        <div>
                            ...
                            <input type="number" id="limitecredito" name="limitecredito" value={personaDTO.cliente.limiteCredito} disabled={!isClienteSelected} onKeyDown={handleEnterDown}
                                onChange={() => {
                                    updateCliente("limiteCredito", personaDTO.cliente.limiteCredito || 0)
                                }} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed" />
                        </div>
                        <div className="flex flex-row gap-2">
                    ...
                    <div className="flex flex-col gap-2 w-full">
                        ...
                        <div className="flex flex-col gap-2 p-2 justify-center w-full items-center">
                            ...
                            <Autocomplete
                                label=""
                                options={ListaDePrecios?.map(zona => ({
                                    id: zona.lpCodigo,
                                    label: zona.lpDescripcion,
                                    value: zona.lpCodigo
                                })) ?? []}
                                value={
                                    listaDePreciosSeleccionada
                                        ? {
                                            id: listaDePreciosSeleccionada.lpCodigo,
                                            label: listaDePreciosSeleccionada.lpDescripcion,
                                            value: listaDePreciosSeleccionada.lpCodigo
                                        }
                                        : null
                                }
                                onChange={(option) => {
                                ...
                                onCreateOption={async (value) => {
                                    await handleCrearPrecio(value)
                                    // Return a new Option object as expected by Autocomplete
                                    return {
                                        id: value,
                                        label: value,
                                        value: value
        ...
        <div ref={formRef}>
            <div className="max-w-7xl mx-auto  bg-white">
                ...
                <ModalVendedores
                    ...
                    onSelect={(vendedor) => handleSelectVendedor(vendedor)}
                    setIsOpen={setIsBuscadorVendedoresOpen}
                />
                <ModalVendedores
                    isOpen={isBuscadorAgentesOpen}
                    onSelect={(agente) => handleSelectedAgente(agente)}
                    setIsOpen={setIsBuscadorVendedoresOpen}
                />
                <ModalVendedores
                    isOpen={isBuscadorCobradorOpen}
                    onSelect={(cobrador) => handleSelectedCobrador(cobrador)}
