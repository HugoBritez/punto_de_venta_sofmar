...
export default function Presupuestos() {
  ...
  const calcularTotalExcentas = () => {
    ...
    items.forEach((item) => {
      ...
      totalExentas +=
        exentas.exentas * item.cantidad * (1 - (item.descuentoIndividual ?? 0) / 100);
    });
    return totalExentas;
  };
  ...
  const calcularTotalConDescuentos = () => {
    return items.reduce((acc, item) => {
      const subtotalConDescuentoIndividual =
        item.subtotal * (1 - (item.descuentoIndividual ?? 0) / 100);
  ...
  const presupuestoData = {
    presupuesto: {
      ...
      pre_vencimiento: selectedItem?.al_vencimiento.substring(0, 10)
    ...
    detalle_presupuesto: items.map((item) => {
      ...
      const depre_exentas = Math.max(
        (1 - ((item.descuentoIndividual ?? 0)/100))*item.exentas*item.cantidad - itemDescuento
      );
      const depre_cinco = Math.max(
        (1 - ((item.descuentoIndividual ?? 0)/100))*item.impuesto5*item.cantidad*21 - itemDescuento
      );

      const depre_diez = Math.max(
        (1 - ((item.descuentoIndividual ?? 0)/100))*item.impuesto10*item.cantidad*11 - itemDescuento
      );

      const calcularTotalDescuento = (item.precioUnitario * ((item.descuentoIndividual ?? 0) / 100) -itemDescuento)

      return {
        depre_articulo: item.id,
        depre_cantidad: item.cantidad,
        depre_precio: item.precioUnitario,
        depre_descuento: calcularTotalDescuento,
        depre_exentas: depre_exentas,
        depre_cinco: depre_cinco,
        depre_diez: depre_diez,
        depre_codlote: item.lote,
        depre_lote: item.lote,
  ...
  const finalizarPresupuesto = async () => {
    ...
    try {
      const response = await axios.post(
        `${api_url}presupuestos/agregarPresupuesto`,
        presupuestoData,
        {
          headers: {
      ...
      if (response.data && response.data.body) {
        ...
        Auditar(
          ...
          newPresupuestoID,
          parseInt(operador),
          `Presupuesto ${newPresupuestoID} creado por ${vendedor}`
        );
        toast({
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Ha ocurrido un error al finalizar el presupuesto",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
  ...
  return (
    <div>
      ...
      <Box bg={"gray.100"} h={"100vh"} w={"100%"} p={2}>
          ...
          <Flex flexDirection={isMobile ? "column" : "row"}>
                        ...
                        >
                          <Flex>
                            ...
                            <Text as="span" color="red.500" fontSize={"14px"}>
                              Precio Contado: {formatCurrency(moneda === 'PYG' ? articulo.ar_pvg : articulo.ar_pvd)}
                            </Text>
                            <Minus />
                            <Text as="span" color="red.500" fontSize={"14px"}>
                              Precio Credito:{" "}
            ...
            >
              ...
              <Box pt={2}>
                ...
                <Text fontSize="sm" fontWeight="bold">
                  Total IVA 5%: {formatCurrency(calcularTotal5()*21)}
                </Text>
                <Divider borderWidth={"2px"} borderColor={"blue.500"} my={1} />
                <Text fontSize="sm" fontWeight="bold">
                  Total IVA 10%: {formatCurrency(calcularTotal10()*11)}
                </Text>
              </Box>
              <Box textAlign={isMobile ? "left" : "right"}>
                ...
                <Button
                    ...
                    onClick={() => {
                      handleOpenOtherModal("observaciones");
                    }}
                  >
                    Agregar Obs.
                  </Button>
        ...
        <Modal
          isOpen={activeModal === "observaciones"}
          onClose={handleCloseOtherModal}
          size="6xl"
          isCentered={true}
        >
          ...
          <ModalContent>
            ...
            <ModalCloseButton />
            <ModalBody padding={isMobile ? 2 : 8}>
              <Input
                variant="filled"
