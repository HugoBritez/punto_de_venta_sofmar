  ...
  const generarExcel = async (data: PedidoFaltante[]) => {
    ...
    try {
      ...
      const datosExcel = data.map((pedido) => ({
        ...
        "Marca": pedido.marca,
        "Cant. Pedido": Number(pedido.cantidad_pedido_num),
        "Cant. Faltante": Number(pedido.cantidad_faltante_num),
        "P. Unitario": Number(pedido.p_unitario_num),
        "Subtotal": Number(pedido.subtotal_num),
